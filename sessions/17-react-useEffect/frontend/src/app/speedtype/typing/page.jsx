...
async function saveResults(userId, wpm, accuracy) {
  try {
...
export default function TypingTest({ user }) {
  ...
  function handleKeyPress(event) {
    ...
    // Escape => restart (go back to idle)
    if (key === 'Escape') {
    ...
    // Space starts the test from idle
    ...
    // Ignore everything else unless running
    if (status !== STATUS.RUNNING) return;
  ...
  async function finishTest() {
    ...
    await saveResults(user.id, calculatedWPM, acc);
  }
